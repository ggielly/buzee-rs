...
use crate::ipc::{refresh_ignore_allow_cache, send_message_to_frontend};
use crate::user_prefs::set_scan_running_status;
use log::info;
use std::sync::Mutex;
use std::time::{SystemTime, UNIX_EPOCH};

pub async fn run_sync_operation(
    ...
    if sync_running.0 == "true" || switch_off {
        ...
        tokio::spawn(async move {
            ...
            if detailed_scan_allowed {
                ...
                let files_parsed =
                    parse_content_from_files(&mut new_conn, window.clone(), app.clone()).await;
...
pub async fn add_specific_folders(
    window: &tauri::WebviewWindow,
    file_paths: Vec<String>,
    is_folder: bool,
    app: AppHandle,
