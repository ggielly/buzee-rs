...
use crate::custom_types::{DBStat, DateLimit, Error, QuerySegments};
...
use crate::tantivy_index::{
    acquire_searcher_from_reader, create_tantivy_schema, get_tantivy_index,
    parse_query_and_get_top_docs, return_document_search_results,
};
...
pub fn search_fts_index(
    ...
    // otherwise run the Tantivy search query
        ...
        match get_tantivy_index(create_tantivy_schema()) {
            Ok(tantivy_index) => match acquire_searcher_from_reader(&app) {
                Ok(searcher) => {
