...
use crate::database::models::{AppData, FileTypes, UserPrefs};
use crate::database::schema::{app_data, file_types, user_preferences};
...
pub fn set_scan_running_status(
    ...
    if set_time {
        ...
        let _ = diesel::update(app_data::table)
            .set((
                ...
                app_data::last_scan_time.eq(SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .unwrap()
                    .as_secs() as i64),
            ))
...
pub fn set_manual_setup_flag_in_db(flag: bool, app: &tauri::AppHandle) {
    let mut conn = establish_connection(&app);
    let _ = diesel::update(user_preferences::table)
...
pub fn get_global_shortcut(app: &tauri::AppHandle) -> Shortcut {
    ...
    let shortcut_modifiers =
        combined_modifiers
            .iter()
            .fold(Some(combined_modifiers[0]), |acc, &modifier| {
                Some(match acc {
...
pub fn get_modifiers_and_code_from_global_shortcut(app: &tauri::AppHandle) -> (Modifiers, Code) {
    let (combined_modifiers, key) = set_modifiers_and_code_from_state(app);
    let shortcut_modifiers = combined_modifiers
        .iter()
        .fold(Some(combined_modifiers[0]), |acc, &modifier| {
            Some(match acc {
                Some(acc_value) => acc_value | modifier,
