...
fn get_history_db_path() -> Option<PathBuf> {
    ...
    get_profile_name(&user_directory_path)
...
fn where_clauses(terms: &[&str]) -> String {
    terms
        ...
        .map(|&term| {
            format!(
                "(moz_places.title LIKE '%{}%' OR moz_places.url LIKE '%{}%')",
                term, term
            )
        })
        .collect::<Vec<_>>()
...
fn use_history_search(query: Option<&str>, limit: i64, offset: i64) -> HistoryResult {
    ...
    create_copy_of_firefox_history_database()
        .expect("Could not create a backup of the Firefox history database");
