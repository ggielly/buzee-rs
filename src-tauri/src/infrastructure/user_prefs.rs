use crate::domain::types::{DbPool, UserPreferencesState};
use crate::infrastructure::database::establish_connection;
use crate::infrastructure::database::models::{AppData, FileTypes, UserPrefs};
use crate::infrastructure::database::schema::{app_data, file_types, user_preferences};
use diesel::{Connection, ExpressionMethods, QueryDsl, RunQueryDsl, SqliteConnection};
use std::sync::{Arc, RwLock};

pub fn set_default_user_prefs(conn: &mut SqliteConnection, reset_settings_flag: bool) {
    if reset_settings_flag {
        let _ = diesel::update(user_preferences::table)
            .set((
                user_preferences::first_launch_done.eq(true),
                user_preferences::onboarding_done.eq(true),
                user_preferences::launch_at_startup.eq(true),
                user_preferences::show_in_dock.eq(true),
                user_preferences::global_shortcut_enabled.eq(true),
                user_preferences::global_shortcut.eq("Alt+Space"),
                user_preferences::automatic_background_sync.eq(true),
                user_preferences::detailed_scan.eq(true),
                user_preferences::roadmap_survey_answered.eq(false),
                user_preferences::parse_pdfs.eq(true),
                user_preferences::manual_setup.eq(false),
                user_preferences::enable_logs.eq(false),
                user_preferences::pdf_max_ocr_pages.eq(150),
                user_preferences::ocr_threads.eq(1),
                user_preferences::ocr_sort_order.eq("size_asc"),
            ))
            .execute(conn)
            .unwrap();
    }

    let existing_prefs = user_preferences::table
        .select(user_preferences::id)
        .load::<i32>(conn)
        .expect("Error loading user_prefs");
    if existing_prefs.len() == 0 {
        let new_user_prefs = UserPrefs {
            first_launch_done: true,
            onboarding_done: false,
            show_search_suggestions: true,
            launch_at_startup: true,
            show_in_dock: true,
            global_shortcut_enabled: true,
            global_shortcut: "Alt+Space".to_string(),
            automatic_background_sync: true,
            detailed_scan: true,
            roadmap_survey_answered: false,
            parse_pdfs: true,
            manual_setup: false,
            enable_logs: false,
            pdf_max_ocr_pages: 150,
            ocr_threads: 1,
            ocr_sort_order: "size_asc".to_string(),
        };
        diesel::insert_into(user_preferences::table)
            .values(&new_user_prefs)
            .execute(conn)
            .expect("Error inserting new user_prefs");
    }
}

pub fn set_default_app_data(conn: &mut SqliteConnection) {
    let existing_app_data = app_data::table
        .select(app_data::id)
        .load::<i32>(conn)
        .expect("Error loading app_data");

    if existing_app_data.len() == 0 {
        let new_app_data = AppData {
            app_name: "Buzee".to_string(),
            app_version: "0.1.0".to_string(),
            app_mode: "window".to_string(),
            app_theme: "light".to_string(),
            app_language: "en".to_string(),
            last_scan_time: 0,
            scan_running: false,
        };
        diesel::insert_into(app_data::table)
            .values(&new_app_data)
            .execute(conn)
            .expect("Error inserting new app_data");
    }
}

pub fn get_app_theme(pool: &DbPool) -> String {
    let mut conn = establish_connection(pool);
    app_data::table
        .select(app_data::app_theme)
        .first::<String>(&mut conn)
        .unwrap_or_else(|_| "system".to_string())
}

pub fn set_app_theme_in_db(theme_json: &str, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(app_data::table)
        .set(app_data::app_theme.eq(theme_json.to_string()))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update app theme: {}", e))
        .ok();
}

pub fn set_default_file_types(conn: &mut SqliteConnection) {
    const DOCUMENT_FILETYPES: [&str; 11] = [
        "csv", "docx", "key", "md", "numbers", "pages", "pdf", "pptx", "txt", "xlsx", "xls",
    ];
    const IMAGE_FILETYPES: [&str; 4] = ["jpg", "jpeg", "png", "gif"];
    const BOOK_FILETYPES: [&str; 4] = ["epub", "mobi", "azw3", "pdf"];
    const AUDIO_FILETYPES: [&str; 5] = ["mp3", "wav", "aac", "flac", "ogg"];
    const VIDEO_FILETYPES: [&str; 5] = ["mp4", "mkv", "avi", "mov", "wmv"];

    let existing_file_types = file_types::table
        .select(file_types::id)
        .load::<i32>(conn)
        .expect("Error loading file_types");

    if existing_file_types.len() == 0 {
        let mut new_file_types: Vec<FileTypes> = Vec::new();
        for file_type in DOCUMENT_FILETYPES.iter() {
            new_file_types.push(FileTypes {
                file_type: file_type.to_string(),
                file_type_category: "document".to_string(),
                file_type_allowed: true,
                added_by_user: false,
            });
        }
        for file_type in IMAGE_FILETYPES.iter() {
            new_file_types.push(FileTypes {
                file_type: file_type.to_string(),
                file_type_category: "image".to_string(),
                file_type_allowed: true,
                added_by_user: false,
            });
        }
        for file_type in BOOK_FILETYPES.iter() {
            new_file_types.push(FileTypes {
                file_type: file_type.to_string(),
                file_type_category: "book".to_string(),
                file_type_allowed: true,
                added_by_user: false,
            });
        }
        for file_type in AUDIO_FILETYPES.iter() {
            new_file_types.push(FileTypes {
                file_type: file_type.to_string(),
                file_type_category: "audio".to_string(),
                file_type_allowed: true,
                added_by_user: false,
            });
        }
        for file_type in VIDEO_FILETYPES.iter() {
            new_file_types.push(FileTypes {
                file_type: file_type.to_string(),
                file_type_category: "video".to_string(),
                file_type_allowed: true,
                added_by_user: false,
            });
        }
        new_file_types.push(FileTypes {
            file_type: "folder".to_string(),
            file_type_category: "folder".to_string(),
            file_type_allowed: true,
            added_by_user: false,
        });

        conn.transaction::<_, diesel::result::Error, _>(|conn| {
            diesel::insert_into(file_types::table)
                .values(new_file_types)
                .execute(conn)
        })
        .unwrap();
    }
}

pub fn return_user_prefs_state(prefs: &Arc<RwLock<UserPreferencesState>>) -> UserPreferencesState {
    prefs.read().unwrap_or_else(|e| e.into_inner()).clone()
}

pub fn set_user_preferences_state_from_db_value(prefs: &Arc<RwLock<UserPreferencesState>>, pool: &DbPool) {
    log::info!("Setting user preferences to state");
    let mut conn = establish_connection(pool);
    let user_preferences_from_db = user_preferences::table
        .select((
            user_preferences::first_launch_done,
            user_preferences::onboarding_done,
            user_preferences::show_search_suggestions,
            user_preferences::launch_at_startup,
            user_preferences::show_in_dock,
            user_preferences::global_shortcut_enabled,
            user_preferences::global_shortcut,
            user_preferences::automatic_background_sync,
            user_preferences::detailed_scan,
            user_preferences::roadmap_survey_answered,
            user_preferences::parse_pdfs,
            user_preferences::manual_setup,
            user_preferences::enable_logs,
            user_preferences::pdf_max_ocr_pages,
            user_preferences::ocr_threads,
            user_preferences::ocr_sort_order,
        ))
        .first::<UserPrefs>(&mut conn)
        .expect("Error loading user_prefs");

    let mut state = prefs.write().unwrap_or_else(|e| e.into_inner());
    state.first_launch_done = user_preferences_from_db.first_launch_done;
    state.onboarding_done = user_preferences_from_db.onboarding_done;
    state.show_search_suggestions = user_preferences_from_db.show_search_suggestions;
    state.launch_at_startup = user_preferences_from_db.launch_at_startup;
    state.show_in_dock = user_preferences_from_db.show_in_dock;
    state.global_shortcut_enabled = user_preferences_from_db.global_shortcut_enabled;
    state.global_shortcut = user_preferences_from_db.global_shortcut;
    state.automatic_background_sync = user_preferences_from_db.automatic_background_sync;
    state.detailed_scan = user_preferences_from_db.detailed_scan;
    state.roadmap_survey_answered = user_preferences_from_db.roadmap_survey_answered;
    state.parse_pdfs = user_preferences_from_db.parse_pdfs;
    state.manual_setup = user_preferences_from_db.manual_setup;
    state.enable_logs = user_preferences_from_db.enable_logs;
    state.pdf_max_ocr_pages = user_preferences_from_db.pdf_max_ocr_pages;
    state.ocr_threads = user_preferences_from_db.ocr_threads;
    state.ocr_sort_order = user_preferences_from_db.ocr_sort_order;
}

pub fn fix_global_shortcut_string(new_shortcut_string: String) -> String {
    let mut new_shortcut_code: String = new_shortcut_string.split("+").last().unwrap_or_default().to_string();
    if new_shortcut_code != "Space"
        && !new_shortcut_code.contains("Key")
        && !new_shortcut_code.contains("Digit")
    {
        if new_shortcut_code.chars().all(char::is_alphabetic) {
            let formatted_shortcut_code = format!("Key{}", &new_shortcut_code);
            new_shortcut_code = formatted_shortcut_code;
        } else if new_shortcut_code.chars().all(char::is_numeric) {
            let formatted_shortcut_code = format!("Digit{}", &new_shortcut_code);
            new_shortcut_code = formatted_shortcut_code;
        }
    }

    let mut splits: Vec<&str> = new_shortcut_string.split("+").collect();
    splits.pop();
    splits.push(&new_shortcut_code);
    let new_shortcut_string = splits.join("+");
    log::debug!("new_shortcut_string: {:?}", new_shortcut_string);
    new_shortcut_string.to_string()
}

pub fn set_new_global_shortcut_in_db(new_shortcut_string: String, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::global_shortcut.eq(new_shortcut_string))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_onboarding_done_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::onboarding_done.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_launch_at_startup_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::launch_at_startup.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_show_search_suggestions_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::show_search_suggestions.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_detailed_scan_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::detailed_scan.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_roadmap_survey_answered_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::roadmap_survey_answered.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_parse_pdfs_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::parse_pdfs.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_manual_setup_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::manual_setup.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_enable_logs_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::enable_logs.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn get_enable_logs_flag(conn: &mut SqliteConnection) -> bool {
    use crate::infrastructure::database::schema::user_preferences as prefs;
    match prefs::table.select(prefs::enable_logs).first::<bool>(conn) {
        Ok(flag) => flag,
        Err(_) => false,
    }
}

pub fn get_pdf_max_ocr_pages(prefs: &Arc<RwLock<UserPreferencesState>>) -> i64 {
    return_user_prefs_state(prefs).pdf_max_ocr_pages.max(1)
}

pub fn set_pdf_max_ocr_pages_in_db(pages: i64, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::pdf_max_ocr_pages.eq(pages))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn get_ocr_threads(prefs: &Arc<RwLock<UserPreferencesState>>) -> i64 {
    return_user_prefs_state(prefs).ocr_threads.clamp(1, 4)
}

pub fn get_ocr_sort_order(prefs: &Arc<RwLock<UserPreferencesState>>) -> String {
    return_user_prefs_state(prefs).ocr_sort_order
}

pub fn set_ocr_threads_in_db(threads: i64, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::ocr_threads.eq(threads))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_ocr_sort_order_in_db(order: String, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::ocr_sort_order.eq(order))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_global_shortcut_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::global_shortcut_enabled.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn set_automatic_background_sync_flag_in_db(flag: bool, pool: &DbPool) {
    let mut conn = establish_connection(pool);
    let _ = diesel::update(user_preferences::table)
        .set(user_preferences::automatic_background_sync.eq(flag))
        .execute(&mut conn)
        .map_err(|e| log::error!("Could not update user preference: {}", e))
        .ok();
}

pub fn get_global_shortcut_string(prefs: &Arc<RwLock<UserPreferencesState>>) -> String {
    return_user_prefs_state(prefs).global_shortcut
}

pub fn is_global_shortcut_enabled(prefs: &Arc<RwLock<UserPreferencesState>>) -> bool {
    return_user_prefs_state(prefs).global_shortcut_enabled
}