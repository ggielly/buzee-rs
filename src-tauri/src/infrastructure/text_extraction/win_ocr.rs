    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            OcrError::UnsupportedFormat => {
                write!(f, "image format is not supported by Windows OCR")
            }
...
mod windows_ocr {
    ...
    fn recognize_pdf_parallel(
        ...
        let cached_snapshot: Vec<CachedPdfPage> =
            cached_by_index.values().map(|c| (*c).clone()).collect();
...
mod tests {
    ...
    fn extension_is_lowercased_without_dot() {
        assert_eq!(
            image_extension(Path::new("photo.PNG")),
            Some("png".to_string())
        );
        assert_eq!(
            image_extension(Path::new("doc.jpg")),
            Some("jpg".to_string())
        );
        assert_eq!(
            image_extension(Path::new("archive.tar.gz")),
            Some("gz".to_string())
        );
