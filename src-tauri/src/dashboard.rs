// Backend for the Dashboard home page.
//
// Aggregates the index data into a single `DashboardStats` payload so the
// frontend only has to make one IPC call. All heavy lifting happens here with
// SQL aggregates instead of loading every row into memory.
...
// A single (label, count, total-size) row for the file-type breakdown.
#[derive(QueryableByName, Debug)]
struct BucketCount {
...
/// Per-file-type counts and cumulative sizes.
...
fn top_largest(conn: &mut SqliteConnection, limit: i64) -> Vec<DocumentSearchResult> {
  use crate::database::schema::document::file_type;
  document::table
...
/// Builds the full set of statistics shown on the Dashboard.
pub fn get_dashboard_stats(app: &AppHandle) -> DashboardStats {
  ...
  let total_size = load_scalar(&mut conn, "SELECT COALESCE(SUM(size), 0.0) AS val FROM document");
  ...
  DashboardStats {
    ...
    scan_running: sync_running == "true",
    filetype_counts: buckets.into_iter().map(|(t, n, s)| crate::custom_types::DashboardBuckets {
      file_type: t,
      count: n,
      size_bytes: s,
    }).collect(),
